 ...
 ### DRAW SECTION START ###
   ...
   # feedline = CPW(width, gap, start=DPoint(10e5,10e5), end=DPoint(200e5,200e3))
    ...
    for i in range(10):
        ...
        if i<=4:
            vias = via.viafy_CPW(feedline1, 500e3,1e6,100e3, dest=cell, gnd2gnd_dy=None,  bridge_layer1 = layers[5], bridge_layer2= layers[5], dest2=None, via_left = False, via_right=False,
            number_of_points=10, avoid_points = [], avoid_distances = [], flag = 1)
        else:
            vias = via.viafy_CPW(feedline1, 500e3,1e6,100e3, dest=cell, gnd2gnd_dy=None,  bridge_layer1 = layers[5], bridge_layer2= layers[5], dest2=None, via_left = False, via_right=False,
            number_of_points=10, avoid_points = [], avoid_distances = [], flag = 1.5)
        ...
        if i<=4:
            vias = via.viafy_CPW(feedline2, 500e3,1e6,100e3, dest=cell, gnd2gnd_dy=None,  bridge_layer1 = layers[5], bridge_layer2= layers[5], dest2=None, via_left = False, via_right=False,
            number_of_points=10, avoid_points = [], avoid_distances = [], flag = 2)
        else:
            vias = via.viafy_CPW(feedline2, 500e3,1e6,100e3, dest=cell, gnd2gnd_dy=None,  bridge_layer1 = layers[5], bridge_layer2= layers[5], dest2=None, via_left = False, via_right=False,
            number_of_points=10, avoid_points = [], avoid_distances = [], flag = -1.5)
        ...
        if i<=4:
            vias = via.viafy_CPW(feedline3, 500e3,1e6,100e3, dest=cell, gnd2gnd_dy=None,  bridge_layer1 = layers[5], bridge_layer2= layers[5], dest2=None, via_left = False, via_right=False,
            number_of_points=10, avoid_points = [], avoid_distances = [], flag = 1)
        else:
            vias = via.viafy_CPW(feedline3, 500e3,1e6,100e3, dest=cell, gnd2gnd_dy=None,  bridge_layer1 = layers[5], bridge_layer2= layers[5], dest2=None, via_left = False, via_right=False,
            number_of_points=10, avoid_points = [], avoid_distances = [], flag = 1.5)
        ...
        if i<=4:
            vias = via.viafy_CPW(feedline4, 500e3,1e6,100e3, dest=cell, gnd2gnd_dy=None,  bridge_layer1 = layers[5], bridge_layer2= layers[5], dest2=None, via_left = False, via_right=False,
            number_of_points=10, avoid_points = [], avoid_distances = [], flag = 2)
        else:
            vias = via.viafy_CPW(feedline4, 500e3,1e6,100e3, dest=cell, gnd2gnd_dy=None,  bridge_layer1 = layers[5], bridge_layer2= layers[5], dest2=None, via_left = False, via_right=False,
            number_of_points=10, avoid_points = [], avoid_distances = [], flag = -1.5)
    ...
    position = 7.75e6
    round1 = MetalRounds(100e3, DPoint(position,position))
    round2 = MetalRounds(100e3, DPoint(-position,position))
    round3 = MetalRounds(100e3, DPoint(position,-position))
    round4 = MetalRounds(100e3, DPoint(-position,-position))
    round1.place(cell, layers[5])
    round2.place(cell, layers[5])
    round3.place(cell, layers[5])
    round4.place(cell, layers[5])
    ...
    for point in sample_hole_stepped.empty_box_coords:
        freza = ScrewHoles(550e3, point)
        #freza.place(cell, layers[0], merge=True)
        #freza.place(cell, layers[1], merge=True)
        #freza.place(cell, layers[2], merge=True)
        #freza.place(cell, layers[3], merge=True)
